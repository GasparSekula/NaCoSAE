...
from optimization_utils.scorer import (
    Scorer as S,
    get_text_features,
    get_image_features
)
from optimization_utils.generator import Generator as G
...
def optimize_for_images(
    ...
    if args.init_descriptions_set_size != "all":
        random.seed(0)
        init_sentences = random.sample(init_sentences, int(args.init_descriptions_set_size))
    ...
    for i, image_id in enumerate(image_ids):
        ...
        best_score = sorted(lines_with_scores[f"{image_id}"], 
                            key = lambda x: -x[0])[0]
...
def main(args):
    ...
    if args.ablation:
        ...
        images_ids = random.sample(image_ids, 1000)
